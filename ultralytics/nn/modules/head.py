...
__all__ = "Detect", "MAFDetect", "IDetect", "DDetect", "Detect_ASFF", "Detect_SEAM", "Detect_MultiSEAM", "Segment", "MAFSegment", "ISegment", "DSegment", "Segment_ASFF", "Pose", "MAFPose", "IPose", "DPose", "Pose_ASFF", "Pose_SEAM", "Pose_MultiSEAM", "Classify", "OBB", "MAFOBB", "IOBB", "DOBB", "OBB_ASFF", "RTDETRDecoder", "v10Detect", "v10Detect", "v10IDetect", "v10DDetect"
...
class MAFDetect(nn.Module):
    """MAF-YOLO Detect head for detection models."""
...
class Detect_SEAM(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), SEAM(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
                ...
                nn.Sequential(
                    nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)),
                    nn.Sequential(DWConv(c3, c3, 3), SEAM(c3, c3, 1)),
                    nn.Conv2d(c3, self.nc, 1),
                )
                for x in ch
            )
...
class Detect_MultiSEAM(nn.Module):
    """YOLO Detect head for detection models."""
    ...
    def __init__(self, nc=80, ch=()):
        ...
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), MultiSEAM(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
                ...
                nn.Sequential(
                    nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)),
                    nn.Sequential(DWConv(c3, c3, 3), MultiSEAM(c3, c3, 1)),
                    nn.Conv2d(c3, self.nc, 1),
                )
                for x in ch
            )
...
class DSegment(DDetect):
    """YOLOv9 Segment head for segmentation models."""
    ...
    def forward(self, x):
        """Return model outputs and mask coefficients if training, otherwise return outputs and mask coefficients."""
        p = self.proto(x[-1])  # mask protos
        bs = p.shape[0]  # batch size

        mc = torch.cat([self.cv4[i](x[i]).view(bs, self.nm, -1) for i in range(self.nl)], 2)  # mask coefficients
        x = DDetect.forward(self, x[:-1])
        if self.training:
            return x, mc, p
        return (torch.cat([x, mc], 1), p) if self.export else (torch.cat([x[0], mc], 1), (x[1], mc, p))
    
    
class Segment_ASFF(Detect_ASFF):
...
class DOBB(DDetect):
    """YOLOv9 OBB detection head for detection with rotation models."""
    ...
    def __init__(self, nc=80, ne=1, ch=()):
        ...
        c4 = max(ch[0] // 4, self.ne)
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.ne, 1)) for x in ch[:-1])
...
class DPose(DDetect):
    """YOLOv9 Pose head for keypoints models."""
    ...
    def __init__(self, nc=80, kpt_shape=(17, 3), ch=()):
        ...
        c4 = max(ch[0] // 4, self.nk)
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.nk, 1)) for x in ch[:-1])
...
class Pose_MultiSEAM(Detect_MultiSEAM):
    """YOLOv8 Pose head for keypoints models."""
    ...
    def forward(self, x):
        ...
        bs = x[0].shape[0]  # batch size
        kpt = torch.cat([self.cv4[i](x[i]).view(bs, self.nk, -1) for i in range(self.nl)], -1)  # (bs, 17*3, h*w)
        x = Detect_MultiSEAM.forward(self, x)
        if self.training:
            return x, kpt
        pred_kpt = self.kpts_decode(bs, kpt)
        return torch.cat([x, pred_kpt], 1) if self.export else (torch.cat([x[0], pred_kpt], 1), (x[1], kpt))
